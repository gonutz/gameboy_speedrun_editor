...
type Window interface {
	...
	// CreateImage creates a new image with the given name and size. You can
	// call the DrawImage... functions with that name. After CreateImage, the
	// content of the image is undefined (it may be any color or, if you
	// re-create the same image multiple times it may contain the last image
	// data that was put into the image).
	// Use SetImagePixels to fill the new image with data.
	CreateImage(path string, width, height int) error

	// SetImagePixels fills an existing image with RGBA data. In case the image
	// does not exist (use CreateImage first to create it) it returns an error.
	SetImagePixels(path string, rgba []byte) error

	// TODO Think about this:
	// SetImagePixelsRGBA(path string, rgba []byte)
	// SetImagePixelsBGRA(path string, bgra []byte)
