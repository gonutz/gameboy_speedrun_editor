...
const (
	...
	windowedStyle = w32.WS_OVERLAPPEDWINDOW | w32.WS_VISIBLE
...
func (w *window) CreateImage(path string, width, height int) error {
	if tex, ok := w.textures[path]; ok {
		if tex.width == width && tex.height == height {
			return nil
		} else {
			// The existing texture in the cache has the wrong size, so we
			// delete it to re-create it below.
			tex.texture.Release()
			tex.sysTexture.Release()
			delete(w.textures, path)
		}
	}
	...
	// TODO In the other texture creation function, also remember the
	// sysTexture.
...
func (w *window) SetImagePixels(path string, rgba []byte) error {
	tex, ok := w.textures[path]
	if !ok {
		return fmt.Errorf("prototype.draw.Window.SetImagePixels: image '%s' does not exist", path)
	}

	if len(rgba) != tex.width*tex.height*4 {
		return fmt.Errorf(
			"prototype.draw.Window.SetImagePixels: "+
				"image '%s' of size %dx%d needs %d bytes of RGBA data, "+
				"but the function was given %d bytes instead.",
			path, tex.width, tex.height, tex.width*tex.height*4,
			len(rgba),
		)
	}

	// TODO This right now destroys the original rgba. Either
	// 1. Revert it at the end, so rgba is the same as before (bad for parallel
	//    updating of rgba).
	// 2. Make a copy and change that.
	// 3. Make this fact clear in the documentation.
	for i := 0; i < len(rgba); i += 4 {
		rgba[i], rgba[i+2] = rgba[i+2], rgba[i]
	}

	// TODO Give error messages more context.
	rect, err := tex.sysTexture.LockRect(0, nil, d3d9.LOCK_DISCARD)
	if err != nil {
	...
	if err := w.device.UpdateTexture(tex.sysTexture, tex.texture); err != nil {
		return errors.New("d3d9.Device.UpdateTexture: " + err.Error())
	}

	tex.texture.GenerateMipSubLevels()
